...
# -------------------------------------------------------------------
# Тесты на корректные поисковые запросы
# -------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        "hi",                     # простое слово
        "hello world",            # два слова с пробелом
        "python-telegram bot",    # слово с дефисом и пробелом
        "что такое async",        # русский текст + английское слово
        "test, with punctuation!",# текст с пунктуацией
        "numbers 123",            # текст с цифрами
        "dots.and,commas",        # текст с точками и запятыми
...
def test_validate_search_query_valid(query):
    ...
    Функция должна возвращать True для текстов, которые:
    - содержат буквы, цифры, пробелы и базовую пунктуацию
    - имеют длину от 2 до 200 символов
    """
    assert validate_search_query(query) is True


# -------------------------------------------------------------------
# Тесты на некорректные поисковые запросы
# -------------------------------------------------------------------
...
def test_validate_search_query_invalid(query):
    ...
    Функция должна возвращать False для текстов, которые:
    - содержат запрещённые символы (emoji, HTML)
    - слишком короткие (<2 символов) или слишком длинные (>200 символов)
    - содержат только пробелы или управляющие символы
    """
    assert validate_search_query(query) is False


# -------------------------------------------------------------------
# Тест на поведение при None
# -------------------------------------------------------------------
